...
export default function AdminDashboardPage() {
  ...
  const [approvalTab, setApprovalTab] = useState<"sellers" | "products">("sellers");
        ...
        <div className="lg:col-span-1 bg-white border border-slate-200 rounded-2xl p-6 shadow-sm flex flex-col">
          ...
          <div className="flex border-b border-slate-100 mb-4 text-xs font-bold">
            <button
              ...
              className={[
                "flex-1 pb-3 text-center transition-all border-b-2 cursor-pointer bg-transparent",
                approvalTab === "sellers"
                  ? "border-[#006c49] text-[#006c49]"
                  : "border-transparent text-slate-500 hover:text-slate-700",
              ].join(" ")}
            >
              Duyệt Người Bán ({pendingSellersList.length})
            </button>
            <button
            ...
            >
              Duyệt Sản Phẩm ({pendingProductsList.length})
            </button>
          </div>
